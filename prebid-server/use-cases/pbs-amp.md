...
{: .alert.alert-info :}
**Note:** the `prebidrubicon` and `prebidmsftpsp` AMP vendor strings define slightly different parameters; Microsoft uses "PLACEMENT_ID" as the argument to rtc-config while Rubicon uses "REQUEST_ID". They both translate to `tag_id` when passed to Prebid Server.
