...
[Cookie syncs](./cookie-syncs.html) save the user ID for each Bidder in the cookie, and each Bidder's ID is sent back to that Bidder during the [auction](../endpoints/openrtb2/auction.html).
Prebid Server does not use this ID for any other reason.

## IDs during Auction

The [`/openrtb2/auction`](../endpoints/openrtb2/auction.html#gdpr) endpoint accepts `user.regs.gdpr` and `user.ext.consent` fields,
...
The [`POST /cookie_sync`](../endpoints/cookieSync.html) endpoint accepts `gdpr` and `gdpr_consent` properties in the request body.
...
The [`/setuid`](../endpoints/setuid.html) endpoint accepts `gdpr` and `gdpr_consent` query params. This endpoint
...
host company can decide whether it behaves like a `1` or `0` through the [app configuration](./configuration.html).
