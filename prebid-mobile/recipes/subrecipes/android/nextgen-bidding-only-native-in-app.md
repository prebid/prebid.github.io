---
layout: page_v2
title: Android Next-Gen SDK Bidding-Only Integration - Native In-App
description: Android Next-Gen SDK Bidding-Only Integration - Native In-App
sidebarType: 2
---

# Android Next-Gen SDK Bidding-Only Integration - Native In-App
...
3. Ad requests are made to Google Ad Manager.
...
private fun createAd() {
    ...
    val adRequest = NativeAdRequest.Builder(AD_UNIT_ID, listOf(NativeAd.NativeAdType.CUSTOM_NATIVE))
...
Load the native ad using `NativeAdLoader`:
...
## Step 4: Configure and make a Next-Gen SDK ad request
{:.no_toc}

Prepare the `NativeAdRequest` and run an ad request using `NativeAdLoader.load()` as described in the Next-Gen SDK docs for the [native ads](https://developers.google.com/admob/android/next-gen/native). Build the request from the same `NativeAdRequest.Builder` object you passed to `fetchDemand`.

If the `NativeAdRequest` contains targeting keywords, the respective Prebid line item will be returned from GAM. Be sure to use the same builder object. Otherwise, the ad request won't contain targeting keywords, and Prebid's ad won't ever be displayed.

## Step 5: Implement NativeAdLoaderCallback
{:.no_toc}

In order to capture the native ad response, you will need to implement `NativeAdLoaderCallback`.
