...
Prebid introduced the concept of optional targeting keys with 4.23. The way this is accomplished is CONSTANTS.DEFAULT_TARGETING_KEYS is now a subset of CONSTANTS.TARGETING_KEYS. When a publisher defines targetingControls.allowTargetingKeys, this replaces the constant CONSTANTS.DEFAULT_TARGETING_KEYS and can include optional keys defined only in CONSTANTS.TARGETING_KEYS. The first example of this is hb_adomain, which is optional but set to bid.meta.advertiserDomains[0]. A publisher may want to set this to get better information from their ad server about VAST errors, to set floors on certain buyers, to monitor the volume from a buyer, or to help track down malvertising and poorly formed creatives. 
...
| AD_ID | `hb_adid` | yes |
| PRICE_BUCKET | `hb_pb` | yes |
| SIZE | `hb_size` | yes |
| DEAL | `hb_deal` | yes |
| SOURCE | `hb_source` | yes |
| FORMAT | `hb_format` | yes |
| UUID | `hb_uuid` | yes |
| CACHE_ID | `hb_cache_id` | yes |
| CACHE_HOST | `hb_cache_host` | yes |
| ADOMAIN | `hb_adomain` | no |
...
```
Below is an example config containing `allowTargetingKeys` adding optional key hb_adomain and excluding all default targeting keys except `hb_bidder`, `hb_adid`, `hb_size` and `hb_pb`:
