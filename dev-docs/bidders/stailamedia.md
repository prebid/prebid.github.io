...
layout: bidder
title: Stailamedia
description: Prebid Stailamedia Bidder Adaptor
...
<a name="stailamedia-bid-params" />

{: .alert.alert-danger :}
All Stailamedia placements included in a single call to `requestBids` must belong to the same parent Publisher.  If placements from two different publishers are included in the call, the Stailamedia bidder will not return any demand for those placements. <br />
*Note: This requirement does not apply to adapters that are [aliasing](/dev-docs/publisher-api-reference/aliasBidder.html) the Stailamedia adapter.*
...
|---------------------|----------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|-------------------------------------------------------|------------------|
| `placement_id` | required | The placement ID from Stailamedia.  You may identify a placement using the `invCode` and `member` instead of a placement ID. **Prebid Server requires an integer value.**                                                    | `234234`                                            | `integer`         |
| `member`                                        | optional | The member ID  from Stailamedia. Must be used with `invCode`.                                                                                                                    | `'12345'`                                             | `string`         |
| `invCode`                                       | optional | The inventory code from Stailamedia. Must be used with `member`.                                                                                                                 | `'abc123'`                                            | `string`         |
| `publisherId`                                   | optional | The publisher ID from Stailamedia. It is used by the Stailameida end point to identify the publisher when placement id is not provided and `invCode` goes wrong.                                                                                                                    | `12345`                                             | `integer`         |
| `frameworks`                                    | optional | Array of integers listing API frameworks for Banner supported by the publisher. | `integer` |
| `user`                                          | optional | Object that specifies information about an external user. See [User Object](#stailamedia-user-object) for details.                                                               | `user: { age: 25, gender: 0, dnt: true}`              | `object`         |
...
| `use_pmt_rule` | optional | If `true`, Stailamedia will return net price to Prebid.js after publisher payment rules have been applied.                                                                       | `true`                                                | `boolean`        |
| `keywords`                                      | optional | A set of key-value pairs applied to all ad slots on the page.  Mapped to [buy-side segment targeting](https://monetize.xandr.com/docs/segment-targeting) (login required). A maximum of 100 key/value pairs can be defined at the page level. Each tag can have up to 100 additional key/value pairs defined. Values can be empty. See [Passing Keys Without Values](#stailamedia-no-value) below for examples. If you want to pass keywords for all adUnits, see [Auction Level Keywords](#stailamedia-auction-keywords) for an example. Note that to use keyword with the Prebid Server adapter, that feature must be enabled for your account by an account manager. | `keywords: { genre: ['rock', 'pop'] }`                | `object`         |
| `video`                                         | optional | Object containing video targeting parameters.  See [Video Object](#stailamedia-video-object) for details.                                                                        | `video: { playback_method: ['auto_play_sound_off'] }` | `object`         |
| `app`                                           | optional | Object containing mobile app parameters.  See the [App Object](#stailamedia-app-object) for details.                                                                      | `app : { id: 'app-id'}`                               | `object`         |
...
| `generate_ad_pod_id`                            | optional | Signal to Stailamedia to split impressions by ad pod and add unique ad pod id to each request. Specific to long form video endpoint only. Supported by Prebid Server.  | `true`                                                | `boolean`        |

<a name="stailamedia-video-object" />
...
<a name="stailamedia-user-object" />
...
<a name="stailaedia-app-object" />

#### App Object

Stailamedia supports using prebid within a mobile app's webview. If you are interested in using an SDK, please see [Prebid Mobile]({{site.baseurl}}/prebid-mobile/prebid-mobile.html) instead.
...
Stailamedia returns custom keys that can be sent to the adserver through bidderSettings: buyerMemberId, dealPriority, and dealCode. The following snippet demonstrates how to add these custom keys as key-value pairs.
    ...
    adserverTargeting: [
      {
        ...
        val: function(bidResponse) {
          return bidResponse.stailamedia.buyerMemberId;
        }
      },
      {
        key: "apn_prio", // Use key configured in your adserver
        val: function(bidResponse) {
          return bidResponse.stailamedia.dealPriority;
        }
      }, {
        key: "apn_dealcode", // Use key configured in your adserver
        val: function(bidResponse) {
          return bidResponse.stailamedia.dealCode;
        }
      }
    ]
  }
...
<a name="stailamedia-no-value" />

#### Passing Keys Without Values

It's possible to use the `keywords` parameter to define keys that do not have any associated values. Keys with empty values can be created in Prebid.js and can also be sent through Prebid Server to Stailamedia. The following are examples of sending keys with empty values:
...
<a name="stailamedia-fpd" />

#### First Party Data

Publishers should use the `ortb2` method of setting [First Party Data](https://docs.prebid.org/features/firstPartyData.html).

At this time however, the `stailamedia` bidder only reads the First Party Data when using the Prebid Server and Prebid Server Premium endpoints.  The client-side version of the `stailamedia` bidder does not use the values from the First Party Data fields.

PBS/PSP supports all first party data fields: site, user, segments, and imp-level first party data.


<a name="stailamedia-amp" />

#### User Sync in AMP

If you are syncing user id's with Prebid Server and are using Stailamedia' managed service, see [AMP Implementation Guide cookie-sync instructions](/dev-docs/show-prebid-ads-on-amp-pages.html#user-sync) for details.

<a name="stailamedia-debug-auction" />

#### Mobile App Display Manager Version

The Stailamedia endpoint expects `imp.displaymanagerver` to be populated for mobile app sources
...
the empty `displaymanagerver` before sending the request to Stailamedia.

#### Debug Auction

{: .alert.alert-danger :}
Enabling the Stailamedia Debug Auction feature should only be done for diagnosing the Stailamedia auction. Do not enable this feature in a production setting where it may impact users.
...
server-side Stailamedia adapter. This example includes an `imp` object with an Stailamedia test placement ID and sizes
