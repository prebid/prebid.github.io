...
| `splitMultiFormat`  | optional | If `true`, when a multi-format request is made - the request made to the AN endpoint will be split into separate requests (one for each mediaType).  This is to help ensure distinct settings needed for each media type are set properly. Defaults to `true` | `true` | `boolean` |
