...
| `allowSmallerSizes`<br/>(Prebid.js only)| optional | If `true`, ads smaller than the values in your ad unit's `sizes` array will be allowed to serve. Defaults to `false`.                                                         | `true`                                                | `boolean`        |
...
| `keywords`          | optional | A set of key-value pairs applied to all ad slots on the page.  Mapped to [buy-side segment targeting](https://console.appnexus.com/docs/segment-targeting) (login required). Values can be empty. See [Passing Keys Without Values](#appnexus-no-value) below for examples. Note: keywords must be enabled by your account manager if to be used on Prebid Server.| `keywords: { genre: ['rock', 'pop'] }`                | `object` (Prebid.js)<br/> `array of objects` (Server) |
