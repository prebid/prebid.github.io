...
- <a name="allowUnknownBidderCodes" /> When `allowUnknownBidderCodes` is `true`, bidders that have not been explicitly requested in [`adUnit.bids`](../adunit-reference.html#adunitbids) may take part in the auction. This can break custom logic that relies on the availability of a bid request object for any given bid; in particular, "unknown" bidders will not trigger a [`bidRequested`](getEvents.html) event and will have an undefined `payload.bidRequest` in [MASS custom renderers](../modules/mass.html#configuration-parameters).
