...
3. Select Prebid [module](/dev-docs/prebid-mobile/modules/modules-overview)
...
Once you have a [Prebid Server](/dev-docs/prebid-mobile/prebid-mobile-getting-started), you will add 'account' info to the Prebid Mobile.
...
:::warning
Starting from PrebidMobile `3.0.0` the setCustomPrebidServer() method and the `Host.Appnexus` and `Host.Rubicon` enums have been removed. Please check the server URL in [API changes](/dev-docs/prebid-mobile/updates-3.0/ios/api-changes#host) and use `PrebidMobile.initializeSdk` (below) to set the Prebid Server URL.
:::
...
your Prebid Server team, use the [arbitrary OpenRTB](/dev-docs/prebid-mobile/pbm-api/ios/pbm-targeting-ios#arbitrary-openrtb) method like this:
...
```swift
Prebid.initializeSDK(PREBID_SERVER_URL, gadMobileAdsVersion: string(for: MobileAds.shared.versionNumber)) { status, error in
    switch status {
    case .succeeded:
...
```swift
Prebid.initializeSDK(PREBID_SERVER_URL, gadMobileAdsVersion: GADGetStringFromVersionNumber(GADMobileAds.sharedInstance().versionNumber) { status, error in
    switch status {
    case .succeeded:
        print("Prebid SDK successfully initialized")
    case .failed:
...
See the [global parameters page](/dev-docs/prebid-mobile/pbm-api/ios/pbm-targeting-ios) for details.
...
:::warning
Starting from PrebidMobile `3.0.0` the `prebidServerHost` property and the `setCustomPrebidServer` method are removed. Use `Prebid.initializeSDK` instead. If you used `Host.Appnexus` or `Host.Rubicon` to set your host, you should check the server URL in [API changes](/dev-docs/prebid-mobile/updates-3.0/ios/api-changes#host).
:::
...
- [GAM using Original API](code-integration-ios)
- [No Ad Server](/dev-docs/prebid-mobile/modules/rendering/ios-sdk-integration-pb)
- [GAM using Rendering API](/dev-docs/prebid-mobile/modules/rendering/ios-sdk-integration-gam)
- [AdMob](/dev-docs/prebid-mobile/modules/rendering/ios-sdk-integration-gam)
- [AppLovin MAX](/dev-docs/prebid-mobile/modules/rendering/ios-sdk-integration-max)
