...
The Plugin Renderer feature does not work with [GAM Original API](/dev-docs/prebid-mobile/pbm-api/ios/ios-sdk-integration-gam-original-api) since the ad rendering does not happen in the Prebid SDK but externally. Despite that if you are using the regular GAM integration it will work fine.
