...
For interstitials only, these define which sizes Prebid Server will choose to send to bidders. See [Prebid Server interstitial support](/dev-docs/prebid-server/endpoints/openrtb2/pbs-endpoint-auction#interstitial-support). If this option is used, you'll need to set the size to 1x1.
