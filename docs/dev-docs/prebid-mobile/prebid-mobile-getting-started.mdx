...
:::info
If this is your first time working with header bidding, we recommend that you read [What is Prebid?](/overview/intro) before diving into Prebid Mobile.
:::
...
See the [Prebid Server documentation](/prebid-server/overview/prebid-server-overview) for more information on [setting up your own server host](/prebid-server/hosting/pbs-hosting).
...
Working with your Prebid Server host, you will need to create at least one "account settings" block. Prebid Server calls this a [top-level stored request](/prebid-server/features/pbs-storedreqs). Each top-level stored request contains parameters that are global to the entire auction, not just
...
- [How Prebid Server works with Prebid SDK](/prebid-server/use-cases/pbs-sdk)
