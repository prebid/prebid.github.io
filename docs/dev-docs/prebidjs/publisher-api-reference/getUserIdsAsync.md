...
`getUserIdsAsync()` returns a promise to the same value returned by [getUserIds()](/dev-docs/prebidjs/next/publisher-api-reference/getUserIds), but it's guaranteed to resolve only once the complete set of IDs is available:
