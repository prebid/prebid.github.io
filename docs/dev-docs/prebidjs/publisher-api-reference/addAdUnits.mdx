...
Takes one ad unit object or an array of ad unit objects and adds them to the Prebid auction.  For usage examples, see [Examples](#addAdUnits-Examples) below and the [Getting Started](/dev-docs/getting-started) page.
...
| `labelAny` | optional  | array[string] | An array of string labels, used for showing responsive ads.  With the `labelAny` operator, just one label has to match for the condition to be true. Works with the `sizeConfig` object passed in to [pbjs.setConfig](/dev-docs/publisher-api-reference/setConfig).  |
| `labelAll` | optional  | array[string] | An array of string labels, used for showing responsive and conditional ads. With the `labelAll` conditional, every element of the target array must match an element of the label array in order for the condition to be true. Works with the `sizeConfig` object passed in to [pbjs.setConfig](/dev-docs/publisher-api-reference/setConfig).  |
...
Note that `bids` is optional only for [Prebid Server stored impressions](/dev-docs/modules/prebidServer#stored-imp), and required in all other cases.


| Name     | Scope    | Type          | Description                                                                                                                                                                       |
|----------+----------+---------------+-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `bidder` | Required | String        | Bidder code. Find the [complete reference for all supported bidders here](/dev-docs/bidders).                                                                                          |
| `params` | Required | Object        | Bidder's preferred way of identifying a bid request. Find the [complete reference for all supported bidders here](/dev-docs/bidders).                                                  |
| `labelAny` | optional  | array[string] | An array of string labels, used for showing responsive ads.  With the `labelAny` operator, just one label has to match for the condition to be true. Works with the `sizeConfig` object passed in to [pbjs.setConfig](/dev-docs/publisher-api-reference/setConfig).  |
| `labelAll` | optional  | array[string] | An array of string labels, used for showing responsive and conditional ads. With the `labelAll` conditional, every element of the target array must match an element of the label array in order for the condition to be true. Works with the `sizeConfig` object passed in to [pbjs.setConfig](/dev-docs/publisher-api-reference/setConfig).  |
...
For the list of properties please visit the [adUnit.video reference](/dev-docs/adunit-reference#adUnit.video).
...
For an example of a native ad unit, see below.  For more detailed instructions, see [Show Native Ads](/dev-docs/show-native-ads).
...
For an example of an instream video ad unit where the integration is handled on your own, see below.  For more detailed instructions, see [Show Video Ads](/dev-docs/show-video-with-a-dfp-video-tag).
...
For an example of an outstream video ad unit, see below.  For more detailed instructions, see [Show Outstream Video Ads](/dev-docs/show-outstream-video-ads).
...
For an example of a banner ad unit, see below.  For more detailed instructions, see [Getting Started](/dev-docs/getting-started).
