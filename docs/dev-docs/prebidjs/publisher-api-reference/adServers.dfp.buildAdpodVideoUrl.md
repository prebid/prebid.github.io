...
:::caution
For long form Prebid.js will add key-value strings for multiple bids. This prevents retrieving the description url from bid.
:::
...
:::warning
Set the `pbjs.setConfig.cache.url` to the URL that will cache the VAST XML. 
:::
