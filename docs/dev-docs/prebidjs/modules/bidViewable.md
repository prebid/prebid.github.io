...
- The module does not work with adservers other than GAM and only with GPT integration. See the [other Bid Viewable Event](/dev-docs/modules/bidViewableIO) module for an ad server independent version.
...
:::warning
This feature doesn't work with [Instream Video](/dev-docs/examples/instream-banner-mix), as GPT's impressionViewable event is not triggered for instream-video-creative
:::
...
- [Building a PBJS analytics adapter](/dev-docs/integrate-with-the-prebid-analytics-api)
- [Building a PBJS bidder adapter](/dev-docs/bidder-adaptor)
