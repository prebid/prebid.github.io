...
It works by translating them into an equivalent US national string as detailed in [Interpreting USNat strings](/features/mspa-usnat#interpreting-usnat-strings), and using it to apply the same [activity restrictions](/features/mspa-usnat#usnat-activity-restrictions). Prebid.js encourages publishers to consider with their legal counsel use of the national string over state strings. New state strings are being released, yet national string support is mature and less error prone.
...
By default, including this module enables activity restrictions for GPP sections from ID 8 to 12, as long as the [consentManagementGpp](/dev-docs/modules/consentManagementGpp) module is also included and configured.
...
This module provides a [normalization algorithm](/features/mspa-usnat#interpreting-usnat-strings) for each section from 8 to 12, which it uses to translate each section's data into its equivalent section 7 (usnat) representation.
...
- [Prebid Consent Management - GPP Control - USNat module](/dev-docs/prebidjs/next/modules/gppControl_usnat)
- [Prebid Activity Controls -- GPP control module - USNat](/dev-docs/prebidjs/next/modules/gppControl_usnat)
