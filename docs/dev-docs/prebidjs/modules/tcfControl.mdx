...
:::warning
This module requires the [TCF consent management module](/dev-docs/prebidjs/next/modules/consentManagementTcf) (the base consent module), which reads consent values from the Consent Management Platform (CMP). The TCF Control Module
will then take action based on the results. See the [base module page](/dev-docs/prebidjs/next/modules/consentManagementTcf) for general background, usage, and legal disclaimers.
:::

## Overview

The [base consent module](/dev-docs/prebidjs/next/modules/consentManagementTcf) performs the following actions:
...
The following fields related to anonymizing aspects of the auction are supported in the [`consentManagement`](/dev-docs/prebidjs/next/modules/consentManagementTcf) object:
...
* <a id="strictStorageEnforcement"></a> By default, Prebid allows some limited use of storage even when purpose 1 consent was not given: this is limited to non-PII, such as [category translation mappings](/dev-docs/prebidjs/next/modules/categoryTranslation), or temporary test data used to probe the browser's storage features. If `strictStorageEnforcement` is true, Purpose 1 consent will always be enforced for any access to storage.
...
* [EU GDPR Consent Management Module](/dev-docs/prebidjs/next/modules/consentManagementTcf)
...
* [CMP Best Practices](/dev-docs/prebidjs/next/cmp-best-practices)
