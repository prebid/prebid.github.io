...
* Define the prebid options in the header of the document where the Brightcove Player will be located.
  ...
  * These options can include any of the options supported in **[Prebid Options](/dev-docs/prebidjs/next/plugins/bc/bc-prebid-plugin-prebid-options)**
...
* Call `renderAd()`  to tell the plugin to render the selected ad
  * Pass in desired rendering options as listed in **[Render Options](/dev-docs/prebidjs/next/plugins/bc/bc-prebid-plugin-render-options)**
...
Using the `adServerCallback` option described in **[Prebid Options](/dev-docs/prebidjs/next/plugins/bc/bc-prebid-plugin-prebid-options)**, you can instruct the plugin to pass the results of the prebidding process to your preferred ad server.  You can then pass the results from your ad server back to the plugin by calling `renderAd()` as described in the **[Brightcove Prebid Plugin API](/dev-docs/prebidjs/next/plugins/bc/bc-prebid-plugin-api)**.  The plugin will render the final results in the Brightcove Player.  In this case, you MUST pass in a value for the `creative` parameter.
...
* Define the options that are being passed into the plugin in the body of the document where the Brightcove Player will be located.
  * These options can include any of the options supported in **[Prebid Options](/dev-docs/prebidjs/next/plugins/bc/bc-prebid-plugin-prebid-options)** as well as the options supported in the **[Render Options](/dev-docs/prebidjs/next/plugins/bc/bc-prebid-plugin-render-options)**.
...
* Define the render options that are being passed into the plugin in the body of the document where the Brightcove Player will be located.
  * These options are described in the **[Render Options](/dev-docs/prebidjs/next/plugins/bc/bc-prebid-plugin-render-options)**.
* Register the plugin by calling `init()`.
* Call `renderAd()`  to tell the plugin to render the selected ad.
...
See **[Plugin Integration with Brightcove Player Using Brightcove Player Studio](/dev-docs/prebidjs/next/plugins/bc/bc-prebid-plugin-integration-studio)**
