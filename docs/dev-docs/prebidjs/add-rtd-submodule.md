...
TODO fix image URL [Prebid RTD Architecture Diagram](/images/prebid-rtd-architecture.jpg)
...
2. Your sub-module may update the reqBidsConfigObj and hit the callback. To inject data into the bid requests, you should follow one of these conventions:
    1. Recommended: use one of these [First Party Data](/features/firstPartyData) conventions:
...
:::warning
Before version 7, the pattern for first party data inspection and enrichment by RTD modules was `getConfig({ortb2])` / `mergeConfig({ortb2})`. With the introduction of [auction-specific data](/features/firstPartyData#supplying-auction-specific-data) in 7, the global `getConfig('ortb2')` is "frozen"
