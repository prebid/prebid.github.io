...
There are many privacy regulations that Prebid publishers need to accommodate. Prebid supplies [modules](/dev-docs/faq#how-does-prebid-support-privacy-regulations) to help Publishers implement their legal policies, but there are scenarios where extra control is needed:
...
`allowActivities` is a new option to [setConfig](/dev-docs/publisher-api-reference/setConfig). It contains a list of activity names -- see the [full list of activities below](#activities). Each activity is an object that can contain these attributes:
...
| `syncUser` | A bid adapter wants to fetch a [user sync](/dev-docs/publisher-api-reference/setConfig#setConfig-Configure-User-Syncing) | User sync is skipped | [`syncType`, `syncUrl`](#params-syncUser) |
...
* Modules can set activity control rules, e.g. [usersync](/dev-docs/publisher-api-reference/setConfig#setConfig-Configure-User-Syncing), [bidderSettings](/dev-docs/publisher-api-reference/bidderSettings), the [GPP](/dev-docs/modules/consentManagementGpp) or [GDPR](/dev-docs/modules/tcfControl) modules. Rules set by modules have a less urgent priority of 10.
...
|------|------|-------------|---------------|
| `componentType` | String | All activities | One of: `'bidder'`, `'userId'`, `'rtd'`, `'analytics'`, or `'prebid'`; identifies the type of component (usually a module) that wishes to perform the activity. `'prebid'` is reserved for Prebid core itself and a few "privileged" modules such as the [PBS adapter](/dev-docs/modules/prebidServer). |
...
| `adapterCode`   | String | All activities | If `componentType` is `'bidder'`, and `componentName` is an [alias](/dev-docs/publisher-api-reference/aliasBidder), then `adapterCode` is the bidder code that was aliased; or identical to `componentName` if the bidder is not an alias. This is undefined when the component is not a bidder.|
| `configName`    | String | <a id="params-fetchBids"></a> `fetchBids`    | When the Prebid Server adapter is part of an auction, this is the name given to its [s2s configuration](/dev-docs/modules/prebidServer), if any. |
...
This is similar to the 'vendor exception' feature of the [TCF Control Module](/dev-docs/modules/tcfControl). This would always allow bidderA to participate in the auction, even without explicit consent in GDPR scenarios. It might indicate, for instance, that this is a 'first party bidder'.
...
the [USNat module](/dev-docs/modules/gppControl_usnat).
...
This approach works in conjunction with other activity-control compiant modules (like the [GPP USNat module](/dev-docs/modules/gppControl_usnat).
...
* [FAQ: How does Prebid.js support privacy regulations](/dev-docs/faq#how-does-prebid-support-privacy-regulations)
