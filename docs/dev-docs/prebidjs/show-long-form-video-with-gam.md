...
:::caution
Ensure your ad ops team has set up line items in Google Ad Manager.
:::
...
:::warning
Make sure you replace the `placementId` in the sample with a proper `placementId`.
:::
...
:::warning
Set the `pbjs.setConfig.cache.url` to the URL that will return the cached VAST XML. 
:::
...
:::caution
For `adpod`, Prebid.js will add key-value strings for multiple bids. This prevents retrieving the description url from bid.
:::
...
[Prebid API Reference](/dev-docs/publisher-api-reference)  
[Prebid.js Video Overview](/prebid-video/video-overview)  
[Prebid.js Long Form (Ad Pod) Video](/prebid-video/video-long-form)
