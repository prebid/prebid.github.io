...
We recommend including the [jwplayerVideoProvider](/dev-docs/prebidjs/next/modules/jwplayerVideoProvider) to connect Prebid.js to your JW Player instance via the [Prebid Video Module](/dev-docs/prebidjs/next/modules/video). 
If you are not using the JW Player Video Provider, we suggest including the JW Player Real Time Data Provider [jwplayerRtdProvider](/dev-docs/prebidjs/next/modules/jwplayerRtdProvider) in order to include JW Player's contextual ad targeting segments in your bid requests. 
...
We recommend populating as many params as possible in `adUnit.mediaTypes.video`. When using the [jwplayerVideoProvider](/dev-docs/prebidjs/next/modules/jwplayerVideoProvider), these fields are populated automatically. 
...
In release 4.30 and later, publishers who are not using the [jwplayerVideoProvider](/dev-docs/prebidjs/next/modules/jwplayerVideoProvider) or [jwplayerRtdProvider](/dev-docs/prebidjs/next/modules/jwplayerRtdProvider) can use the ortb2 method of setting First Party Data. 
