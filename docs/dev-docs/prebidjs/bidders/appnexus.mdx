...
*Note: This requirement does not apply to adapters that are [aliasing](/dev-docs/prebidjs/next/publisher-api-reference/aliasBidder) the AppNexus adapter.*
...
AppNexus supports using prebid within a mobile app's webview. If you are interested in using an SDK, please see [Prebid Mobile](/dev-docs/prebid-mobile) instead.
...
If you are syncing user id's with Prebid Server and are using AppNexus' managed service, see [AMP Implementation Guide cookie-sync instructions](/dev-docs/prebidjs/next/show-prebid-ads-on-amp-pages#user-sync) for details.
