...
- [Prebid Server - Java](/dev-docs/prebid-server/versions/pbs-versions-java)
...
- Look over the [features in each version](/dev-docs/prebid-server/features/pbs-feature-idx) and see if there are any available in only one version that are important to you.
