...
Prebid Server supports a mechanism for Publisher control for overriding privacy-sensitive activities. See the [Activity Controls](/dev-docs/prebid-server/features/pbs-activitycontrols) for more information.
...
1. (done for PBS-Java) GPP infrastructure - the ability to plug new regulations into PBS, and the first sub-module, the [US General Privacy Module](/dev-docs/prebid-server/features/pbs-usgen).

## US Compliance

See [Prebid US Compliance Support](/features/mspa-usnat) for more info.

There are two modules offered by Prebid Server to process GPP string sections 7-12:

1. The [USGen Privacy Module](/dev-docs/prebid-server/features/pbs-usgen) is a high
performance option for interpreting the GPP strings as described in the [Prebid US Compliance reference](/features/mspa-usnat). (PBS-Java only)
1. The [US Custom Logic Privacy Module](/dev-docs/prebid-server/features/pbs-uscustomlogic) is a flexible way for publishers to define their own interpretation of GPP string sections 7-12.

Also note that publishers can consider utilizing [Activity Controls](/dev-docs/prebid-server/features/pbs-activitycontrols). For PBS-Java, the `gppSid`, `geo`, and `gpc` conditions may be useful tools within a compliance strategy.
...
[Activity Controls](/dev-docs/prebid-server/features/pbs-activitycontrols) for
more information.
...
* [Prebid Server Feature Matrix](/dev-docs/prebid-server/features/pbs-feature-idx)
