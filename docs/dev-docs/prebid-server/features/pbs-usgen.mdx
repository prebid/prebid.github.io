...
See the [Prebid US Compliance Support](/features/mspa-usnat) page for
...
1. Utilize the [US Custom Logic Privacy Module](/dev-docs/prebid-server/features/pbs-uscustomlogic). This lets publishers define their own interpretation in JSONLogic format.
...
"`USGen`" is a 'privacy module' called by the [Activity Control system](/dev-docs/prebid-server/features/pbs-activitycontrols) for a specific scenario. e.g.
...
allowed. Unlike the similarly-named [Prebid.js USNat module](/dev-docs/prebidjs/next/modules/gppControl_usnat),
...
1. For each GPP SID defined in the request
    ...
    1. Else pull that section of out the GPP string and process it
        1. If the SID is 8-12, "normalize" the flags to the SID 7 form as described in the Prebid [US Compliance reference](/features/mspa-usnat).
...
In order to utilize this test though, you will need an analytics adapter that can read the Activity Infrastructure log. See [privacy module analytics](/dev-docs/prebid-server/developers/add-a-privacy-module-java#privacy-trace-log-and-analytics) for more detail.
...
* [US Custom Logic Privacy Module](/dev-docs/prebid-server/features/pbs-uscustomlogic)
* [Activity Control system](/dev-docs/prebid-server/features/pbs-activitycontrols)
