...
1. [AMP](/dev-docs/prebid-server/use-cases/pbs-amp) requires that only ad server targeting be returned, not the whole creative body. When chosen, the creative is obtained from network.
1. [Mobile applications](/dev-docs/prebid-server/use-cases/pbs-sdk) could accept the full creative body, but it's better performance for users if ad creatives stay in the network unless they're going to be displayed.
...
![Prebid Caching Architecture](/images/prebid-server/pbs-cache-feature.png)
...
From one of the [video instream examples](/examples/video/instream/jwplayer/pb-ve-jwplayer-platform):
