...
There are many privacy regulations for Prebid publishers to consider. Prebid Server supplies [several privacy features](/dev-docs/prebid-server/features/pbs-privacy) to help Publishers implement their legal policies.
...
only privacy module that plugs into this system is the [USGen Privacy Module](/dev-docs/prebid-server/features/pbs-usgen). There will be more.
...
1. Activity config may instruct the server to call privacy modules like [US General privacy](/dev-docs/prebid-server/features/pbs-usgen).
1. If the Activity Control `denies` the activity, the action will be suppressed/anonymized.
1. However, if the Control `allows` the activity, the system will still go on to check [other relevant privacy activities](/dev-docs/prebid-server/features/pbs-privacy) like TCF/GDPR.
...
|------|------|-------------|---|
| TCF-EU/GDPR | Original [PBS GDPR support](/dev-docs/prebid-server/features/pbs-privacy#gdpr) | no | n/a |
| COPPA | Original [PBS COPPA support](/dev-docs/prebid-server/features/pbs-privacy#coppa) | no | n/a |
| USP | Original [PBS USP/CCPA support](/dev-docs/prebid-server/features/pbs-privacy#ccpa--us-privacy) | no | n/a |
| US General Privacy | [USGen privacy module](/dev-docs/prebid-server/features/pbs-usgen) supports the IAB's GPP SIDs 7-12. | yes | iab.usgeneral |
...
|----------------|-------------|---------------------------|
| `syncUser` | The [/cookie_sync](/dev-docs/prebid-server/endpoints/pbs-endpoint-cookieSync) or [/setuid](/dev-docs/prebid-server/endpoints/pbs-endpoint-setuid) endpoint has been asked to perform a sync or set-cookie. | Sync or setuid is skipped for one or more bidders. |
...
| `reportAnalytics` | The [/auction](/dev-docs/prebid-server/endpoints/openrtb2/pbs-endpoint-auction), [/amp](/dev-docs/prebid-server/endpoints/openrtb2/pbs-endpoint-amp), or [/event](/dev-docs/prebid-server/endpoints/pbs-endpoint-event) endpoint is about to call an analytics adapter. | Adapter is not called. |
...
Modules that perform any 'potentially restricted activity' are responsible for confirming they are allowed to perform that activity. See [Adding a PBS Module](/dev-docs/prebid-server/developers/add-a-module) for more information.

## Further Reading

* [Prebid Server privacy regulation support](/dev-docs/prebid-server/features/pbs-privacy)
* [Prebid Server USGen privacy module](/dev-docs/prebid-server/features/pbs-usgen)
