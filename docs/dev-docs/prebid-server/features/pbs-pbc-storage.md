...
This reference is describes how Prebid Server [module](/dev-docs/prebid-server/pbs-modules/) authors can take advantage of storing data needed by their module.
...
2. Using data cache via PBC: Check the [Storage endpoints](/dev-docs/prebid-server/endpoints/pbs-endpoints-pbc#storage). This approach provides a centralized caching solution that will be available from each PBS instance configured to connect to the PBC application.
...
- [Prebid Server Modules](/dev-docs/prebid-server/pbs-modules/)
