...
- However, there are additional schema attributes supported in Prebid Server, e.g. country and deviceType. See [floor schema dimensions](/dev-docs/prebid-server/features/pbs-floors#floor-schema-dimensions) below for details.
...
:::info
The PBS floors feature isn't a formal [PBS module](/dev-docs/prebid-server/pbs-modules/). Yeah, that bums us out too.
...
![Floors High Level](/images/prebid-server/pbs-floors-basic.png)
...
![Prebid Server Architecture](/images/prebid-server/pbs-floors-detailed.png)
...
1. **Prepare bidder-specific values** - for bidders with [cpm adjustments](/dev-docs/prebid-server/endpoints/openrtb2/pbs-endpoint-auction#bid-adjustments), update imp[].bidfloor with the appropriate value.
...
mediatypes, sizes, and other details if they support it. See the [Bid Adapter Floor Interface](/dev-docs/prebid-server/features/pbs-floors#bid-adapter-floor-interface) section below.

## Floor Enforcement

The enforcement stage of the Floors feature validates bid responses from the adapters.

1. **Let bid adjustments happen as usual** - if the [cpm adjustments](/dev-docs/prebid-server/endpoints/openrtb2/pbs-endpoint-auction#bid-adjustments) feature is active, PBS-core runs it as usual.
...
It may be necessary to build a [server-side analytics adapter](/dev-docs/prebid-server/developers/pbs-build-an-analytics-adapter) in order
...
- [PBS-Go](/dev-docs/prebid-server/developers/add-new-bidder-go)
- [PBS-Java](/dev-docs/prebid-server/developers/add-new-bidder-java)
...
See the [developer's guide to building an analytics adapter](/dev-docs/prebid-server/developers/pbs-build-an-analytics-adapter) for more details, and see existing code as an example.
