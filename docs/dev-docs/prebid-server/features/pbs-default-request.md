...
The `file.name` option is the path/filename of a JSON file containing default request JSON. See the [stored request doc](/dev-docs/prebid-server/features/pbs-storedreqs) and the [openrtb2/auction endpoint doc](/dev-docs/prebid-server/endpoints/openrtb2/pbs-endpoint-auction) for details about the syntax.
