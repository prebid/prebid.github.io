...
This is an endpoint similar to the [Prebid Cache /cache](/dev-docs/prebid-server/endpoints/pbs-endpoints-pbc#post-cache) endpoint, but PBS adds an `Impression` tag to the supplied VAST.
...
- [Endpoint overview](/dev-docs/prebid-server/endpoints/pbs-endpoint-overview)
