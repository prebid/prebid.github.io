...
Host companies may want to set up Prebid Cache to be able to store [module](/dev-docs/prebid-server/pbs-modules/) or other data separately from the main bids/VAST data store. Having a separate data store for each module or a group of modules allows the host company to:
...
- [PBS endpoint overview](/dev-docs/prebid-server/endpoints/pbs-endpoint-overview)
- [PBS data storage](/dev-docs/prebid-server/features/pbs-pbc-storage)
