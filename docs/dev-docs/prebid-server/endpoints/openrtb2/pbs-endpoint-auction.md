...
| cur | 2.5 | Only the first array element is taken to be the "Ad Server Currency" for purposes of [currency conversion](/dev-docs/prebid-server/features/pbs-currency). |
...
Note: the request parameter `ext.prebid.createtids` can override the [transmitTids activity](/dev-docs/prebid-server/features/pbs-activitycontrols).
...
For backwards compatibility the following strings will also be allowed as price granularity definitions. There is no guarantee that these will be honored in the future. "One of ['low', 'med', 'high', 'auto', 'dense']" See [price granularity definitions](/adops/price-granularity).
...
For publishers that use Prebid Server as part of their [GDPR/TCF](/dev-docs/prebid-server/features/pbs-privacy#gdpr) strategy, it can be important to define the Global Vendor List (GVL) ID with an alias.
...
`request.imp[i].ext.prebid.storedrequest` incorporates a [Stored Request](/dev-docs/prebid-server/features/pbs-storedreqs) from the server.
...
For more information, see the docs for [Stored Requests](/dev-docs/prebid-server/features/pbs-storedreqs).
...
Prebid Server adapters can support the [Prebid.js User ID modules](/dev-docs/prebidjs/modules/userId) by reading the following extensions and passing them through to their server endpoints:
...
Publishers can constrain which bidders receive which user.ext.eids entries. See the [Prebid.js user ID permissions](/dev-docs/prebidjs/modules/userId#permissions) reference for background.
...
The request can contain the global `createtids` flag to control the `transmitTid` [Activity Control](/dev-docs/prebid-server/features/pbs-activitycontrols).
...
See the [endpoint documentation on IDs](/dev-docs/prebid-server/endpoints/openrtb2/pbs-endpoint-auction#ids) for details on how PBS creates the various IDs including transaction IDs.
...
This provides additional information for [modules](/dev-docs/prebid-server/pbs-modules/).
...
See Prebid.org [troubleshooting pages](/troubleshooting/pbs-troubleshooting#manual-specification-of-stored-responses-in-openrtb) for how to utilize this feature within the context of the browser.
...
An OpenRTB extension, whether in the the original request or the [stored-request](/dev-docs/prebid-server/features/pbs-storedreqs), can customize the ad server targeting generated by PBS.
...
See the [Prebid Server Floors Feature](/dev-docs/prebid-server/features/pbs-floors) for more info.
...
[Analytics Tags](/dev-docs/prebid-server/developers/module-atags) (aka "aTags") are an advanced feature that some [modules](/dev-docs/prebid-server/pbs-modules/) and
...
See the [Analytics Tags](/dev-docs/prebid-server/developers/module-atags) page for more information.
...
The value in seatbid[].bid[].price may be converted for currency and adjusted with a [bid adjustment factor](/dev-docs/prebid-server/endpoints/openrtb2/pbs-endpoint-auction#bid-adjustments).
...
| imp[]<wbr>.ext<wbr>.prebid<wbr>.options<wbr>.echovideoattrs | Causes PBS-core to [echo video attributes](/dev-docs/prebid-server/endpoints/openrtb2/pbs-endpoint-auction#echo-storedrequest-video-attributes) on seatbid[].bid[].ext.prebid.storedrequestattributes so the player has access to them<br>ex: `true` | boolean | yes |
| imp[]<wbr>.ext<wbr>.prebid<wbr>.storedauctionresponse | PBS-Core skips the auction and uses the response in the DB instead, see [stored responses](/dev-docs/prebid-server/endpoints/openrtb2/pbs-endpoint-auction#stored-responses). | object | no |
| imp[]<wbr>.ext<wbr>.prebid<wbr>.storedbidresponse | PBS-Core calls the adapter with the response in the DB instead of actually running the auction,see [stored responses](/dev-docs/prebid-server/endpoints/openrtb2/pbs-endpoint-auction#stored-responses). | object | no |
| imp[]<wbr>.ext<wbr>.prebid<wbr>.storedrequest<wbr>.id | Look up the defined stored request and merge the DB contents with this imp, see [stored requests](/dev-docs/prebid-server/endpoints/openrtb2/pbs-endpoint-auction#stored-requests). | object | no (yes with [issue 2292](https://github.com/prebid/prebid-server/issues/2292) |
| imp[]<wbr>.ext<wbr>.prebid<wbr>.is_rewarded_inventory | (deprecated) Passed through to bid adapters, see [rewarded video](/dev-docs/prebid-server/endpoints/openrtb2/pbs-endpoint-auction#rewarded-video). (use imp.rwdd in ORTB 2.6) | integer | yes |
...
| device<wbr>.ext<wbr>.prebid<wbr>.interstitial | PBS-core will adjust the sizes on a request for interstitials,see [interstitial support](/dev-docs/prebid-server/endpoints/openrtb2/pbs-endpoint-auction#interstitial-support). | object | yes |
| user<wbr>.ext<wbr>.prebid<wbr>.buyeruids | An alternate to [/cookie_sync](/dev-docs/prebid-server/endpoints/pbs-endpoint-cookieSync), the request can supply bidder ID values, see [buyer uid](#buyer-uid). | object | no |
| ext<wbr>.prebid<wbr>.adservertargeting | advanced targeting value rules, see [custom targeting](/dev-docs/prebid-server/endpoints/openrtb2/pbs-endpoint-auction#custom-targeting). | object | no |
| ext<wbr>.prebid<wbr>.auctiontimestamp | Timestamp for use in correlating PBJS and PBS events. | long int | yes |
| ext<wbr>.prebid<wbr>.aliases | defines alternate names for bidders, see [bidder aliases](/dev-docs/prebid-server/endpoints/openrtb2/pbs-endpoint-auction#bidder-aliases). | object | no |
| ext<wbr>.prebid<wbr>.aliasgvlids | defines the global vendor list IDs for aliases, see [bidder aliases](/dev-docs/prebid-server/endpoints/openrtb2/pbs-endpoint-auction#bidder-aliases). | object | no |
| ext<wbr>.prebid<wbr>.bidadjustmentfactors | Adjust the CPM value of bidrequests, see [bid adjustments](/dev-docs/prebid-server/endpoints/openrtb2/pbs-endpoint-auction#bid-adjustments) | object | no |
| ext<wbr>.prebid<wbr>.bidderconfig | bidder-specific first party data, see [first party data](/dev-docs/prebid-server/endpoints/openrtb2/pbs-endpoint-auction#first-party-data-support). | array of obj | no |
| ext<wbr>.prebid<wbr>.bidderparams | Publishers can specify any adapter-specific cross-impression attributes, see [global bid parameters](/dev-docs/prebid-server/endpoints/openrtb2/pbs-endpoint-auction#global-bid-adapter-parameters) | object | no |
| ext<wbr>.prebid<wbr>.cache | defines whether to put bid results in Prebid Cache, see [cache bids](/dev-docs/prebid-server/endpoints/openrtb2/pbs-endpoint-auction#cache-bids). | object | no |
...
| ext<wbr>.prebid<wbr>.currency<wbr>.rates | publisher-defined currency conversions, see [currency support](/dev-docs/prebid-server/endpoints/openrtb2/pbs-endpoint-auction#currency-support). | object | yes |
...
| ext<wbr>.prebid<wbr>.data.<wbr>bidders | bidders in scope for bidder-specific first party data, see [first party data](/dev-docs/prebid-server/endpoints/openrtb2/pbs-endpoint-auction#first-party-data-support). | array of strings | no |
| ext<wbr>.prebid<wbr>.data.<wbr>eidpermissions | Allows publishers to define which bidders are allowed to see which extended IDs, see [eid permissions](/dev-docs/prebid-server/endpoints/openrtb2/pbs-endpoint-auction#eid-permissions) | object  | no |
...
| ext<wbr>.prebid<wbr>.multibid | allows bidders to respond with more than one bid, see [multi-bid](/dev-docs/prebid-server/endpoints/openrtb2/pbs-endpoint-auction#multibid) | object | yes, but only their value |
...
| ext<wbr>.prebid<wbr>.schains | Bidder-specific supply chains, see [supply chain support](/dev-docs/prebid-server/endpoints/openrtb2/pbs-endpoint-auction#supply-chain-support) | object | no |
| ext<wbr>.prebid<wbr>.targeting | defines the key-value pairs that PBS-core places in seatbid.bid.ext.prebid.targeting, see [ad server targeting](/dev-docs/prebid-server/endpoints/openrtb2/pbs-endpoint-auction#targeting) | object | no |
...
| ext<wbr>.prebid<wbr>.analytics<wbr>.options.<wbr>enableclientdetails | Requests that [aTags](/dev-docs/prebid-server/developers/module-atags) be sent to the client-side for analytics. | boolean | no |
...
| ext<wbr>.prebid<wbr>.bidders | Publishers can specify an adapter-specific flag, see [global bid parameters](/dev-docs/prebid-server/endpoints/openrtb2/pbs-endpoint-auction#global-bid-adapter-parameters) | object | seen on ext<wbr>.prebid<wbr>.bidderparams<wbr>.bidder |
| ext<wbr>.prebid<wbr>.createtids | (PBS-Java only) Override the `transmitTids` activity to influence [which IDs PBS will generate](/dev-docs/prebid-server/endpoints/openrtb2/pbs-endpoint-auction#ids). | boolean | yes |
| ext<wbr>.prebid<wbr>.storedauctionresponse | PBS-Core skips the auction and uses the response in the JSON instead, see [stored responses](/dev-docs/prebid-server/endpoints/openrtb2/pbs-endpoint-auction#stored-responses). | object | no |
...
| seatbid[]<wbr>.bid[]<wbr>.ext<wbr>.prebid<wbr>.meta<wbr>.ATTR | bidder-supplied metadata, see [building a bid adapter](/dev-docs/prebid-server/developers/add-new-bidder-go) | object |
| seatbid[]<wbr>.bid[]<wbr>.ext<wbr>.prebid<wbr>.storedrequestattributes | results of the ext.prebid.options.echovideoattrs option above, see [echo video attributes](/dev-docs/prebid-server/endpoints/openrtb2/pbs-endpoint-auction#echo-storedrequest-video-attributes). | object |
| seatbid[]<wbr>.bid[]<wbr>.ext<wbr>.prebid<wbr>.targeting | Ad server targeting values related to req ext.prebid.targeting. See [ad server targeting](/dev-docs/prebid-server/endpoints/openrtb2/pbs-endpoint-auction#ad-server-targeting) | object |
| seatbid[]<wbr>.bid[]<wbr>.ext<wbr>.prebid<wbr>.type | Type of the bid creative, either "banner", "video", "native", or "audio". | string |
| seatbid[]<wbr>.bid[]<wbr>.ext<wbr>.origbidcpm | Copy of the unadjusted bid price, see [original bid cpm](/dev-docs/prebid-server/endpoints/openrtb2/pbs-endpoint-auction#original-bid-cpm). | float |
| seatbid[]<wbr>.bid[]<wbr>.ext<wbr>.origbidcur | Copy of the original bid currency, see [original bid cpm](/dev-docs/prebid-server/endpoints/openrtb2/pbs-endpoint-auction#original-bid-cpm). | string |
| ext<wbr>.debug | debug mode: useful output, see [debug output](/dev-docs/prebid-server/endpoints/openrtb2/pbs-endpoint-auction#debug-output). | object |
...
- [Example auction request](/dev-docs/prebid-server/endpoints/openrtb2/auction-request-example)
