...
going to utilize in order to control which bidders get into the [/cookie_sync](/dev-docs/prebid-server/endpoints/pbs-endpoint-cookieSync) response when you're running the `coopSync` flag.
...
- [Hosting a Prebid Server Cluster](/dev-docs/prebid-server/hosting/pbs-hosting)
