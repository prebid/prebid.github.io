...
![Prebid Server Modularity Architecture](/images/prebid-server/module-example.png)
...
If your module either utilizes or supplies user-level data like User First Party Data or precise geographic information, it must adhere to the framework supplied by the [Activity Controls](/dev-docs/prebid-server/features/pbs-activitycontrols).
...
- The [ORTB2 Blocking module](/dev-docs/prebid-server/pbs-modules/ortb2-blocking) creates ATags informing analytics adapters that
...
See the [Module Analytics Tag Conventions](/dev-docs/prebid-server/developers/module-atags) for more specific details
...
- PBS-Java: see [Adding a PBS-Java module](/dev-docs/prebid-server/developers/add-a-module-java)
- PBS-Go: see [Adding a PBS-Go module](/dev-docs/prebid-server/developers/add-a-module-go)

Other rules for open source PBS pull request:

- Unit test coverage must exceed 90%.
- A maintainer email address must be provided and be a group, not an individual. e.g. [support@example.com](mailto:support@example.com) rather than [jsmith@example.com](mailto:jsmith@example.com)
...
- [PBS Activity Controls](/dev-docs/prebid-server/features/pbs-activitycontrols)
