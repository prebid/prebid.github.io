...
{:start="4"}
4. The bidder-specific endpoints read the users' cookie for the bidder's domain and respond with a redirect back to Prebid Server's [`/setuid` endpoint](/dev-docs/prebid-server/endpoints/pbs-endpoint-setuid) . This allows that endpoint to read the 3rd party cookie and reflect it back to Prebid Server. Note that if this user doesn't yet have an ID in that 3rd party domain, the sync endpoint is expected to create one.
...
Where Prebid.js isn't present, like on [AMP](/dev-docs/prebid-server/use-cases/pbs-amp) pages, the call to [/cookie_sync](../endpoints/pbs-endpoint-cookieSync) doesn't happen automatically.
...
{:start="3"}
3. At runtime, the `load-cookie` script calls the Prebid Server [/cookie_sync endpoint](/dev-docs/prebid-server/endpoints/pbs-endpoint-cookieSync). The rest works similar to what's described for Prebid.js above. One difference is that the bidders must be manually added to the call. Another difference is that AMP doesn't support iframe syncs, so load-cookie passes instructions to PBS so only pixel syncs are returned.
...
usersync:
   redirect:
      url: https://some-bidder-domain.com/usersync-url?gdpr={{gdpr}}&consent={{gdpr_consent}}&us_privacy={{us_privacy}}&gpp={{gpp}}&gpp_sid={{gpp_sid}}&redirect={{redirect_url}}
      uid-macro: YOURMACRO
```
