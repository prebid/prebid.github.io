...
4. Demand partners return a bid response to PBS. If competitive separation is enabled, PBS performs [category translation](/dev-docs/prebidjs/modules/categoryTranslation) on each bid. Whether category translation is required or not, the bids are stored in prebid cache.
